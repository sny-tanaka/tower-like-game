 ...
 * 等は placeholder の最低値を使う。 baseMachineMaxHp だけ battle slice の値を
 * 渡せるようにしている。
 *
...
export function buildMachineStats({ maxHpNumber }: BuildMachineStatsOpts): MachineStats {
  return {
    ...
    maxHp: BigNum.fromNumber(Math.max(1, maxHpNumber)),
    hpRegen: BigNum.ZERO,
  };
}
