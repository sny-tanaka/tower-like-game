...
// ---------------------------------------------------------------------------
// 通貨ドロップの Tier スケール (02-currencies.md § 3.1)
//
// hybrid 方式:
//   T ≤ 3 : T² (現行と同じ)
//   T ≥ 4 : 9 × 2.25^(T-3) (Tier 上げる旨味を 2.25× で恒久化)
//
// 設計意図: 純 T² だと Tier 上昇 1段あたりの倍率が漸減 (4 → 2.25 → 1.78 → 1.56 …)
// し、敵 HP の伸び (1.8×/Tier) を T4 以降下回るため「Tier 上げると相対的に貧乏になる」
// 状態が発生していた。
// T4 以降を 2.25× 固定にすることで、 現行の T2→T3 で得られていた最大旨味比率
// (9/4 = 2.25) を恒久化する。 敵 HP 1.8×/Tier に対して 報酬は 2.25×/Tier となり、
// 毎 Tier の実質購買力が +25% ずつ伸びる育成感を維持する。
// T1〜T3 は既存バランスを保つため変更しない。
// ---------------------------------------------------------------------------

/**
 * 撃破報酬を Tier スケールで補正した値を返す。
 * - T ≤ 3: baseAmount × T²
 * - T ≥ 4: baseAmount × 9 × 2.25^(T-3) (T=3 の値 9 から 2.25× で外挿)
 */
export function scaledReward(baseAmount: number, tier: number): number {
  if (tier <= 3) {
    return baseAmount * tier * tier;
  }
  return baseAmount * 9 * Math.pow(2.25, tier - 3);
}
