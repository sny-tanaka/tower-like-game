...
describe('ResultDialog', () => {
  ...
  it('status=clear でクリアと表示される', () => {
    render(<ResultDialog {...defaultProps} status="clear" />);
    expect(screen.getByText('クリア')).toBeDefined();
  });

  it('status=gameover で全滅と表示される', () => {
    render(<ResultDialog {...defaultProps} status="gameover" />);
    expect(screen.getByText('全滅')).toBeDefined();
  });

  it('status=retreat で撤退と表示される', () => {
    render(<ResultDialog {...defaultProps} status="retreat" />);
    expect(screen.getByText('撤退')).toBeDefined();
  });

  it('到達 Tier が表示される', () => {
    render(<ResultDialog {...defaultProps} reachedTier={5} />);
    expect(screen.getByText('5')).toBeDefined();
  });

  it('到達 Wave が表示される', () => {
    render(<ResultDialog {...defaultProps} reachedWave={30} />);
    expect(screen.getByText('30')).toBeDefined();
  });

  it('撃破数が表示される', () => {
    render(<ResultDialog {...defaultProps} killed={248} />);
    expect(screen.getByText('248')).toBeDefined();
  });

  it('所要時間が MM:SS 形式で表示される', () => {
    // 312秒 = 05:12
    render(<ResultDialog {...defaultProps} elapsedSec={312} />);
...
describe('ResultDialog 時間フォーマット', () => {
  it('60秒は 01:00 と表示される', () => {
    render(<ResultDialog {...defaultProps} elapsedSec={60} />);
    expect(screen.getByText('01:00')).toBeDefined();
  });

  it('0秒は 00:00 と表示される', () => {
    render(<ResultDialog {...defaultProps} elapsedSec={0} />);
    expect(screen.getByText('00:00')).toBeDefined();
  });

  it('3599秒は 59:59 と表示される', () => {
    render(<ResultDialog {...defaultProps} elapsedSec={3599} />);
